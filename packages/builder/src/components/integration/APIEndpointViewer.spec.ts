...
describe("API Endpoint Viewer", () => {
  ...
  describe("Template mode", () => {
    ...
    it("shows OpenAPI summary instead of operationId for endpoint labels", async () => {
      vi.mocked(restTemplates.get).mockReturnValue({
        id: "postmark",
        name: "Postmark",
        connectionMode: undefined,
        specs: [{ version: "1.0.0", url: "https://example.com/postmark.yaml" }],
      } as any)
      ...
      const menuItem = await waitFor(() => {
        ...
        ).find(el => el.textContent?.includes("Send a single email")) as
          | HTMLElement
          | undefined
        expect(item).not.toBeUndefined()
        return item!
      })
      expect(menuItem.textContent).not.toContain("sendSingleEmail")

      vi.mocked(restTemplates.get).mockReturnValue(undefined)
    })

    it("finds endpoint labels by operationId in endpoint search", async () => {
      vi.mocked(restTemplates.get).mockReturnValue({
        id: "postmark",
        name: "Postmark",
        connectionMode: undefined,
      ...
      await fireEvent.input(searchInput, {
        target: { value: "sendSingleEmail" },
      })

      await waitFor(() => {
        expect(
          Array.from(document.querySelectorAll(".spectrum-Menu-item")).some(
            el => el.textContent?.includes("Send a single email")
          )
        ).toBe(true)
      })
